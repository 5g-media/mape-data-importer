...
from exceptions import InvalidStrTimestamp, NotValidDatetimeTimestamp, MetricNameNotFound, MetricValueNotFound, \
    ...
    VduUuidNotFound
from settings import KAFKA_SERVER, KAFKA_CLIENT_ID, KAFKA_API_VERSION, KAFKA_UC2_QOE_TOPIC, LOGGING, \
    INFLUX_DATABASES, INFLUX_RETENTION_POLICY, KAFKA_UC2_QOE_GROUP_ID
...
def init_kafka_consumer():
    ...
    consumer = KafkaConsumer(bootstrap_servers=KAFKA_SERVER, client_id=KAFKA_CLIENT_ID, enable_auto_commit=True,
                             api_version=KAFKA_API_VERSION, group_id=KAFKA_UC2_QOE_GROUP_ID)
    return consumer
...
def main():
    ...
    consumer.subscribe(pattern=KAFKA_UC2_QOE_TOPIC)
    ...
    for msg in consumer:
        try:
            ...
            temp = {
                "measurement": "qoe_uc2_measurements",
                "time": timestamp,
                "tags": {
                    'vdu_uuid': message['vdu'],
